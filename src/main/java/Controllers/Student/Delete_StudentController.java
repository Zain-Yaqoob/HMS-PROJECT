...
public class Delete_StudentController implements Initializable {
    ...
    private void deleteStudentButtonAction(MouseEvent event) {
        if (dateLeaved.getValue() == null) {
            showAlert("Warning", "Please select a date before deleting", Alert.AlertType.WARNING);
            return;
        }
        ...
        try {
            ...
            pst.executeUpdate();
            showAlert("Message", "Deleted Selected Student", Alert.AlertType.INFORMATION);
            clearFields();
            autoRefresh();
    ...
    private void updateLeavedStudentDB() {
        ...
        try {
            ...
            LocalDate leaveDate = dateLeaved.getValue(); // Get LocalDate from DatePicker
            if (leaveDate != null) {
                String formattedDate = leaveDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
                pst.setString(10, formattedDate);
            } else {
                pst.setString(10, null); // Handle null date
            }
