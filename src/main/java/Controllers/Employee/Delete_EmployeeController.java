...
public class Delete_EmployeeController implements Initializable {
    ...
    private void deleteEmployeeButtonAction(MouseEvent event) {
        ...
        try {
            ...
            pst.executeUpdate();
            showAlert("Message", "Deleted Selected Employee", Alert.AlertType.INFORMATION);
            clearFields();
            autoRefresh();
    ...
    private void updateLeavedEmployeeDB() {
        ...
        try {
            ...
            if (dateLeaved.getValue() == null) {
                showAlert("Warning", "Please select leave date", Alert.AlertType.WARNING);
                return;
            }

            if (dateLeaved.getValue() != null) {
                String formattedDate = dateLeaved.getValue().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
